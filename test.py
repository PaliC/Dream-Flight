...
def max_price_search(max_price, ori):
    try:
        ...
        for x in response["results"]:
            ...
            low_fare_result = requests.get(url)
            print(low_fare_url)
            low_fare_result = loads(low_fare_result.content)
            print(low_fare_result)

            des_airport = low_fare_result[0]["itineraries"][0]["outbound"]["flights"]["destination"]["airport"]
            ...
            to_print = [name_of_airport, name_of_des_city, name_of_des_state, name_of_country, x["price"]]
